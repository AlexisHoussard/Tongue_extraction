...
def inference(data, is_training, batch_norm, scope=''):
    ...
    with slim.arg_scope([slim.conv2d], padding='SAME', normalizer_fn=slim.batch_norm,
        ...
        conv1 = slim.repeat(data, 2, slim.conv2d, 32, [3, 3], scope='conv1',activation_fn=tf.nn.relu)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 3:
        sys.exit('\n\nOnly two arguments (full path to input image and number of chekpoint used) should be given.\n')
    ...
    try:
        ...
        #from ipdb import set_trace
        #set_trace()
        #saver.restore(sess, ckpt.model_checkpoint_path) #Uncomment if you want to predict automatically from last checkpoint
        print(ckpt.model_checkpoint_path)
        n = sys.argv[2]
        saver.restore(sess, ckpt_path + "model.ckpt-" + n) #Uncomment if you want to predict manually from chekcpoint n
        ...
        Image.fromarray(test_image.astype('uint8')).save(output_dir + image_path[-10:-4] + ".png") # saving of the tested images
        Image.fromarray(predicted_mask.astype('uint8')).save(output_dir + image_path[-10:-4] + "-" + n + "_mask.png") #saving of the predicted masks
