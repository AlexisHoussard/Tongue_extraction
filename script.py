...
def image_distortions(image, distortions):
    ...
    #Changement effectue : conversion de mirror (bool tensor) en int tensor
    mirror = tf.less(tf.stack([1.0, distort_left_right_random, 1.0]), 0.5)
    t = [0,1,2]
    sess = tf.compat.v1.Session()
    axis_vector = sess.run(mirror)
    b = tf.boolean_mask(t,axis_vector)
    image = tf.reverse(image, b) # apply an horizontal inversion if distorsions[0] < 0,5
    distort_up_down_random = distortions[1]
    mirror = tf.less(tf.stack([distort_up_down_random, 1.0, 1.0]), 0.5)
    axis_vector = sess.run(mirror)
    b = tf.boolean_mask(t,axis_vector)
    image = tf.reverse(image, b) # apply a vertical inversion if distorsions[1] < 0,5
    return image
...
def distorted_inputs(batch_size, num_epochs):
  #Construct distorted input for  training
    ...
    distortions = tf.random_uniform([2], 0, 1.0, dtype=tf.float32) # generate randomly two numbers between 0 and 1
    
    #---------Comment these two lines to avoid distortions---------------#
...
def train(n_epochs):
  ...
  with tf.Graph().as_default():
    ...
    images, masks =inputs(True,batch_size=batch_size,num_epochs=n_epochs) #images' recovery: use inputs or distorted_inputs
    ...
    summary_writer = tf.compat.v1.summary.FileWriter(tensorb_path, sess.graph)
    try:
        step = 0
        while not coord.should_stop():
            ...
            if step % 500 == 0 :
                saver.save(sess,ckpt_path+ 'model.ckpt',step) #makes a checkpoint every 500 steps 
            if step % 100==0:
